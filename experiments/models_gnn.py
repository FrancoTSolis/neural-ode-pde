...
class MP_PDE_Solver(torch.nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            n_time_windows_out (int): multiple of time windows for output
            use_odeint (bool): whether to use odeint for solving the PDE
        """
        ...
        self.n_time_windows_out = n_time_windows_out
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        if self.use_odeint:
            ...
            # Define ODE function (using derivative network from initializer)
            def ode_func(t, y):
                return self.derivative_net(y)

            # timespan for odeint
            t = torch.linspace(0, 1, (self.n_time_windows_out * self.time_window) + 1).to(h.device)

            # at this point, h has shape [batch * n_nodes, hidden_dim]
            ...
            # pred_z has shape [batch * n_nodes, n_time_windows_out * time_window, hidden_dim]
            out = self.decoding_mlp(pred_z).squeeze(-1)
            # out SHOULD have shape [batch * n_nodes, n_time_windows_out * time_window]
        else:
