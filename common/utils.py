...
class HDF5Dataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, list]:
        ...
        if(f'{self.pde}' == 'CE'):
            ...
            # Normalize both u_base and u_super between 0 and 1
            u_base_min, u_base_max = u_base.min(), u_base.max()
            u_super_min, u_super_max = u_super.min(), u_super.max()

            # Normalize
            u_base = (u_base - u_base_min) / (u_base_max - u_base_min)
            u_super = (u_super - u_super_min) / (u_super_max - u_super_min)
            ...
            # TODO: normalize if ever plan on using WE

            print("WARNING: WE not normalized!!!")
...
class GraphCreator(nn.Module):
    ...
    def create_data(self, datapoints: torch.Tensor, steps: list) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for (dp, step) in zip(datapoints, steps):
            ...
            l = dp[step:self.ntwo * self.tw + step]
