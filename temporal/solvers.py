...
class NeuralODESolver(Iterator):
    """using torchdiffeq.odeint"""
    def __init__(self, rtol=1e-6, atol=1e-6, method='dopri5', device="cpu"): # dopri5: runge-kutta method...
        super().__init__()
        print("initializing neuralODEsolver...")
        self.rtol=rtol
        self.atol=atol
        self.method=method
        self.device=device
    ...
    def __str__(self):
        return f"NeuralODESolver(rtol={self.rtol}, atol={self.atol}, method={self.method})"

    def step(self, h, f, yin, tin):
        """Compute one step of odeint solver
        Args:
            h (torch.Tensor): time increment
            f (Callable[[torch.Tensor], torch.Tensor]: function handle of form f(time, current-solution)
            tin (torch.Tensor): time at start of step
        ...
        Returns:
            torch.Tensor: new solution at time tin + h
        """
        # Create 1D time points array from tin to tin+h
        if tin.dim() > 1:
            tin = tin.squeeze()
        if h.dim() > 1:
            h = h.squeeze()
            
        t_span = torch.tensor([tin.item(), tin.item() + h.item()], device=self.device)
        
        # Wrapper function to match torchdiffeq interface
        def wrapped_f(t, y):
            return f(t, y)
            
        sol = odeint(wrapped_f, yin, t_span, rtol=self.rtol, atol=self.atol, method=self.method)
        return sol