...
def train(args: argparse,
    ...
    for i in range(graph_creator.t_res):
        losses = training_loop(model, unrolling, args.batch_size, optimizer, loader, graph_creator, criterion, device, args.use_odeint)
...
def test(args: argparse,
   ...
   # first we check the losses for different timesteps (one forward prediction array!)
    if args.use_odeint:
        steps = [t for t in range(graph_creator.tw, graph_creator.t_res-(graph_creator.ntwo * graph_creator.tw) + 1)]
        ...
        steps = [t for t in range(graph_creator.tw, graph_creator.t_res-(args.n_time_windows_out * graph_creator.tw) + 1)]
        losses = test_timestep_autoregressive_losses(model=model,
                                                    steps=steps,
                                                    batch_size=args.batch_size,
                                                    ...
                                                    criterion=criterion,
                                                    device=device,
                                                    num_time_windows_out=args.n_time_windows_out)
...
def main(args: argparse):
    ...
    if args.use_odeint:
        odeint_string = 'odeint'
    else:
        odeint_string = 'autoregressive'
    if(args.log):
        logfile = f'experiments/log/{args.model}_{pde}_{odeint_string}_{args.experiment}_xresolution{args.base_resolution[1]}-{args.super_resolution[1]}_n{args.neighbors}_tw{args.time_window}_unrolling{args.unrolling}_time{timestring}.csv'
        print(f'Writing to log file {logfile}')
        sys.stdout = open(logfile, 'w')

    save_path = f'models/GNN_{pde}_{odeint_string}_{args.experiment}_xresolution{args.base_resolution[1]}-{args.super_resolution[1]}_n{args.neighbors}_tw{args.time_window}_unrolling{args.unrolling}_time{timestring}.pt'
    ...
    if args.use_odeint:
        graph_creator_n_tw_out = args.n_time_windows_out
    else:
        graph_creator_n_tw_out = 1
    ...
    graph_creator = GraphCreator(pde=pde,
                                 ...
                                 n_time_windows_out=graph_creator_n_tw_out).to(device)

    if args.model == 'GNN':
        model = MP_PDE_Solver(pde=pde,
                              time_window=graph_creator.tw,
                              eq_variables=eq_variables,
                              n_time_windows_out=args.n_time_windows_out,
                              use_odeint=args.use_odeint).to(device)
    ...
    # for random prediction, plot out final timestep
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_time_windows_out', type=int,
                        default=1, help="Multiple of time windows for output")
    parser.add_argument('--use_odeint', type=eval,
                        default=False, help="Use odeint for solving the PDE")
